...
$
y \sim MVN(0, \alpha K_l + I), K_l: \text{exponential covariance kernel with length scale } l
$
...
Since [`spacelink`](https://hanbyul-lee.github.io/spacelink/reference/spacelink.html) requires normalized input data, we first normalize the count data (`expr_data`) before running the method.
...
We now run [`spacelink`](https://hanbyul-lee.github.io/spacelink/reference/spacelink.html) using the normalized data (`norm_data`) and the spatial coordinates (`coords`).
